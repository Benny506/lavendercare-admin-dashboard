                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-3 sm:px-6 py-3 text-left font-medium text-gray-500">Ticket ID</th>
                                <th className="px-3 sm:px-6 py-3 text-left font-medium text-gray-500">Subject</th>
                                <th className="px-3 sm:px-6 py-3 text-left font-medium text-gray-500">Raised By</th>
                                <th className="px-3 sm:px-6 py-3 text-left font-medium text-gray-500">Priority</th>
                                <th className="px-3 sm:px-6 py-3 text-left font-medium text-gray-500">Status</th>
                                <th className="px-3 sm:px-6 py-3 text-left font-medium text-gray-500">Assignee</th>
                                <th className="px-3 sm:px-6 py-3 text-left font-medium text-gray-500">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                            {ticketsData.map((ticket, idx) => (
                                <tr key={idx} className="cursor-pointer hover:bg-gray-50" onClick={() => handleDetails(ticket)}>
                                    ...
                                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${priorityClass[ticket.priority]}`}>{ticket.priority}</span>
                                    </td>
                                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusClass[ticket.status]}`}>{ticket.status}</span>
                                    </td>
                                    ...
                                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap flex gap-2 items-center">
                                        <button className="text-primary font-medium text-xs sm:text-sm border border-primary rounded px-3 py-1" onClick={e => handleAssign(ticket, e)}>Assign</button>
                                        <button className="text-gray-500 hover:text-primary" onClick={() => handleDetails(ticket)}>ttf</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            {showAssign && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
                    <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md relative animate-fadeIn">
                        <button className="absolute left-4 top-4 text-primary font-medium" onClick={() => setShowAssign(false)}>&lt; Back</button>
                        <h3 className="text-lg font-bold text-center mb-4">Assign Admin</h3>
                        <div className="mb-4">
                            <label className="block mb-2 text-sm font-medium">Choose Admin</label>
                            ...
                            <button className="mt-2 text-primary text-xs font-medium">+ Create Admin role</button>
                        </div>
                        <div className="flex gap-2 mt-6">
                            <button className="flex-1 py-2 rounded-lg bg-gray-100 text-gray-700 font-medium" onClick={() => setShowAssign(false)}>Cancel</button>
                            <button className="flex-1 py-2 rounded-lg bg-primary text-white font-medium">Assign</button>
                        </div>
                    </div>
                </div>
            )}
