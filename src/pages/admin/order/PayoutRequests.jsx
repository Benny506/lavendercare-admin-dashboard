...
const payouts = [
    { vendor: 'Tech Solutions Inc.', user: 'Dr. Emeka Obi', amount: '₦5,200', date: 'May 1, 2025', status: 'Approval' },
    { vendor: 'Creative Designs Co.', user: 'Dr. Emeka Obi', amount: '₦3,800', date: 'August 20, 2025', status: 'Approval' },
    { vendor: 'Global Supplies Ltd.', user: 'Nurse Lillian James', amount: '₦7,500', date: 'July 8, 2025', status: 'Approval' },
    { vendor: 'Invokele Systems LLC', user: 'Doula Funke Adeyemi', amount: '₦2,900', date: 'August 10, 2025', status: 'Pending' },
    { vendor: 'Marketing Pros Agency', user: 'Dr. Emeka Obi', amount: '₦1,500', date: 'Sept 1, 2025', status: 'Rejected' },
];
        ...
        <div className="w-full px-2 md:px-8 py-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-4">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                    <div>
                        <h3 className="font-semibold text-lg">Manage and process vendor payout requests.</h3>
                    </div>
                    ...
                    <table className="min-w-full text-sm">
                        <thead>
                            <tr className="bg-gray-50">
                                ...
                                <th className="py-2 px-2 text-left font-medium">Amount Requested</th>
                                <th className="py-2 px-2 text-left font-medium">Request Date</th>
                                <th className="py-2 px-2 text-left font-medium">Payment Status</th>
                                <th className="py-2 px-2 text-left font-medium">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {payouts.map((p, idx) => (
                                <tr key={p.vendor} className="border-b last:border-b-0 hover:bg-gray-50">
                                    ...
                                    <td className="py-2 px-2">
                                        {p.status === 'Approval' && <span className="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-600">Approval</span>}
                                        {p.status === 'Pending' && <span className="px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-600">Pending</span>}
                                        {p.status === 'Rejected' && <span className="px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-600">Rejected</span>}
                                    </td>
                                    <td className="py-2 px-2 flex gap-2">
                                        {p.status === 'Approval' && (
                                            <>
                                                <button title="Edit" className="text-(--primary-500) hover:underline">
                                                    <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path stroke="#8B8B8A" strokeWidth="2" d="M16.862 5.487a2.06 2.06 0 1 1 2.915 2.914L8.5 19.678l-4 1 1-4 13.277-13.277Z"/></svg>
                                                </button>
                                            </>
                                        )}
                                        {p.status === 'Pending' && (
                                            <>
                                                <button className="bg-green-100 text-green-600 px-2 py-1 rounded text-xs font-semibold">Approve</button>
                                                <button className="bg-red-100 text-red-600 px-2 py-1 rounded text-xs font-semibold">Reject</button>
                                            </>
                                        )}
                                        {p.status === 'Rejected' && (
                                            <>
